...
透過指令列依序要求使用者輸入環境參數，內容包括：
  ...
  4. 顯示選單讓使用者設定連線參數，其選項包括：
      1. 設定 zosmf：依序要求輸入 zosmf 連線 port 和連線編碼，存入 zosmf_port 與 zosmf_encoding。
  ...
  6. 設定 debug：要求輸入 zOpenDebug DPS / RDS 連線 port，存入 debug_dpsPort 與 debug_rdsPort。
...
使用方式:
  ...
  - 用 zosmf_encoding 取代 _ZOSMF_ENCODING_
  ...
  - 用 debug_dpsPort 取代 _DPS_PORT_
  - 用 debug_rdsPort 取代 _RDS_PORT_

更新記錄:
- v2.7.6: 更新版本/日期資訊，並修正文件註解與現行參數一致
...
from configs import load_workspace_config
...
def main():
    ...
    # 檢查是否存在 workspace.yml 設定檔
    workspace_config = load_workspace_config()
    if workspace_config:
        print("發現 install.yml 設定檔，將使用設定檔中的參數。")
        # 從設定檔中讀取參數，覆蓋命令列參數
        if workspace_config.get("auto_continue", False):
            args.yes = True
        if workspace_config.get("workspace", ""):
            args.workspace = workspace_config["workspace"]
    else:
        print("未發現 workspace.yml 設定檔。")
    ...
    properties = {
        # "zosmf": {"port": 443, "encoding": "937"},
        # 針對 POST 連線的預設值
        "zosmf": {"port": 1443, "encoding": "937"},
        "tso": {"codepage": 1047},
        "ssh": {"port": 22},
        "ftp": {"port": 21},
        "rse": {"port": 6800, "encoding": "937"},
    ...
    if workspace_config:
        ...
        # 載入基本參數
        host = workspace_config.get('host', '')
        user = workspace_config.get('user', '')
        password = workspace_config.get('password', '')
        
        # 載入連線參數
        if 'zosmf' in workspace_config:
            properties['zosmf']['port'] = workspace_config['zosmf'].get('port', properties['zosmf']['port'])
            properties['zosmf']['encoding'] = workspace_config['zosmf'].get('encoding', properties['zosmf']['encoding'])
        
        if 'tso' in workspace_config:
            properties['tso']['codepage'] = workspace_config['tso'].get('codepage', properties['tso']['codepage'])
        
        if 'ssh' in workspace_config:
            properties['ssh']['port'] = workspace_config['ssh'].get('port', properties['ssh']['port'])
        
        if 'ftp' in workspace_config:
            properties['ftp']['port'] = workspace_config['ftp'].get('port', properties['ftp']['port'])
        
        if 'rse' in workspace_config:
            properties['rse']['port'] = workspace_config['rse'].get('port', properties['rse']['port'])
            properties['rse']['encoding'] = workspace_config['rse'].get('encoding', properties['rse']['encoding'])
        
        if 'debug' in workspace_config:
            properties['debug']['dpsPort'] = workspace_config['debug'].get('dpsPort', properties['debug']['dpsPort'])
            properties['debug']['rdsPort'] = workspace_config['debug'].get('rdsPort', properties['debug']['rdsPort'])
