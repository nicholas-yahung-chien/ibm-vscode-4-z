...
from configs import load_assistant_config
...
def main():
    ...
    # 檢查是否存在 assistant.yml 設定檔
    assistant_config = load_assistant_config()
    if assistant_config:
        print("發現 assistant.yml 設定檔，將使用設定檔中的參數。")
        # 從設定檔中讀取參數，覆蓋命令列參數
        if assistant_config.get("advanced", {}).get("auto_continue", False):
            args.yes = True
        if assistant_config.get("workspace", ""):
            args.workspace = assistant_config["workspace"]
    else:
        print("未發現 assistant.yml 設定檔，將使用命令列參數或預設值。")
    ...
    # 檢查是否顯示指引資訊
    show_help = True
    if assistant_config and "advanced" in assistant_config:
        show_help = assistant_config["advanced"].get("show_help_info", True)
    ...
    # 優先從設定檔讀取模型選擇
    if assistant_config and assistant_config.get("model_choice"):
        model_choice = assistant_config["model_choice"]
        if 1 <= model_choice <= len(MODEL_OPTIONS):
            selected_model = MODEL_OPTIONS[model_choice - 1]
            print(f"從設定檔讀取模型選擇：{selected_model['display_name']}")
        else:
            print(f"設定檔中的模型選擇 {model_choice} 無效，請手動選擇...")
    ...
    # 優先從設定檔讀取 API Key
    if assistant_config and assistant_config.get("api_key"):
        api_key = assistant_config["api_key"]
        print("從設定檔讀取 API Key Token")
    else:
    ...
    # 優先從設定檔讀取 API Base URL
    if assistant_config and assistant_config.get("api_base_url"):
        api_base = assistant_config["api_base_url"]
        print(f"從設定檔讀取 API Base URL：{api_base}")
    else:
    ...
    # 優先從設定檔讀取 Project ID
    if assistant_config and assistant_config.get("project_id"):
        project_id = assistant_config["project_id"]
        print("從設定檔讀取 Project ID")
    else:
