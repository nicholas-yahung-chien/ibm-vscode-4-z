...
使用方式:
  ...
  可選擇透過 --skip-download 跳過執行 download.py 腳本。

流程說明:
  1. 取得 build.py 腳本所在目錄（考量是否打包為 .exe）。
  2. 根據參數設定工作區（workspace）。
  3. 執行 scripts/download.py，並傳入 --workspace 參數（除非使用 --skip-download 參數）。
...
更新記錄:
- v2.7.1: 新增 --skip-download 參數，允許跳過執行 download.py 腳本
...
def build_executables(scripts_dir):
    ...
    """
    for script in ["install.py", "workspace.py", "update_paths.py", "assistant.py"]:
        print(f"開始打包 {script}...")
...
def parse_arguments():
    ...
    parser.add_argument("--skip-download", action="store_true", help="跳過執行 download.py 腳本")
...
def main():
    ...
    # 1. 執行 download.py 並傳入 --workspace（除非指定跳過）
    if not args.skip_download:
        run_download_py(workspace, scripts_dir)
    else:
        print("跳過執行 download.py 腳本")
