...
def replace_in_file(file_path, pattern, replacement):
    ...
    with open(file_path, "r", encoding="utf-8") as f:
        content = f.read()
    # 注意：若 replacement 含有 Windows 路徑（例如包含 '\u'、'\U'、'\1' 等序列），
    # 直接傳入 re.sub 的 replacement 會被 regex 引擎視為跳脫/反向參照而造成例外：
    #   re.error: bad escape \u ...
    # 使用「function replacement」可讓 replacement 被視為純文字，不再被解析為跳脫序列。
    if callable(replacement):
        new_content = re.sub(pattern, replacement, content)
    else:
        new_content = re.sub(pattern, lambda _m: replacement, content)
